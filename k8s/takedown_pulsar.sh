...
# Print header
echo -e "${GREEN}=== Pulsar Kubernetes Takedown Script ===${NC}"
echo -e "${YELLOW}This script will remove Pulsar from your Kubernetes cluster${NC}"
echo ""
...
if kubectl get namespace pulsar &> /dev/null; then
    ...
    if [ -f "pulsar-config.yaml" ]; then
        ...
        echo -e "${YELLOW}Cannot find pulsar-config.yaml. Attempting direct resource removal...${NC}"
        CONFIG_PATH=""
        PULSAR_PATH=""
    fi
    ...
    if kubectl get statefulset pulsar-broker -n pulsar &> /dev/null; then
        if [ -n "$PULSAR_PATH" ]; then
            kubectl delete -f $PULSAR_PATH -n pulsar
        fi
        if [ $? -ne 0 ]; then
    ...
    if kubectl get configmap pulsar-config -n pulsar &> /dev/null; then
        if [ -n "$CONFIG_PATH" ]; then
            kubectl delete -f $CONFIG_PATH -n pulsar
        fi
        if [ $? -ne 0 ]; then
    ...
    # Remove Pulsar namespace
    echo -e "${YELLOW}Removing Pulsar namespace...${NC}"
    kubectl delete namespace pulsar
    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to remove Pulsar namespace. It might contain resources. Attempting force removal...${NC}"
        kubectl delete namespace pulsar --force --grace-period=0 || true
    fi
    echo -e "${GREEN}Pulsar namespace removed.${NC}"

else
    echo -e "${YELLOW}Pulsar namespace not found.${NC}"
fi

echo -e "${GREEN}Pulsar takedown completed!${NC}"
echo -e "${YELLOW}To verify Pulsar resources have been removed:${NC}"
echo "kubectl get pods -n pulsar"
echo "kubectl get namespaces | grep pulsar"
